...
class commentsController extends AbstractController
{
    ...
    public function submitRating(Request $request, EntityManagerInterface $em, RequestStack $requestStack): Response
    {
        $session = $requestStack->getSession();
        $eventId = $session->get('idEvent'); //
        $user = $em->getRepository(Utilisateur::class)->find(2); // Hardcoded user
        $event = $em->getRepository(Evenement::class)->find($eventId); // Hardcoded event
        ...
        if ($avis) {
            ...
            $avis->setHeure(new \DateTimeImmutable());
    ...
    public function addComment(Request $request, EntityManagerInterface $em, GeminiModerationService $moderator, RequestStack $requestStack): Response
    {
        $session = $requestStack->getSession();
        $eventId = $session->get('idEvent'); //
        $user = $em->getRepository(Utilisateur::class)->find(2);
        ...
        $comment->setTimeComment(new \DateTime());
        $comment->setNbrLikes(0);
        $comment->setNbrDislikes(0);
        $em->persist($comment);
        $em->flush();
    ...
    public function update(Request $request, CommentRepository $commentRepository, int $id, EntityManagerInterface $em, RequestStack $requestStack): Response
    {
        $session = $requestStack->getSession();
        $eventId = $session->get('idEvent');
