...
public function new(Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        $existingType = $entityManager->getRepository(TypeMateriel::class)
            ->findOneBy(['nomm' => $typeMateriel->getNomm()]);

        if ($existingType) {
            // Ajouter un message d'erreur statique
            $this->addFlash('error', 'Le type de matériel "' . $typeMateriel->getNomm() . '" existe déjà.');

            // Rendre le formulaire avec l'erreur
        ...
        // Si pas de doublon, persister et enregistrer
        $entityManager->persist($typeMateriel);
        $entityManager->flush();

        // Rediriger après succès
        $this->addFlash('success', 'Le type de matériel a été ajouté avec succès.');
